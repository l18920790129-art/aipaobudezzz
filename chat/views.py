...
- 历史对话持久化写入 PostgreSQL
- 长期记忆自动更新
- 修复：parsed_params key名统一（agent返回'params'，此处统一读取）
"""
import json
import logging
import re
...
def chat_message(request):
    ...
    try:
        ...
        return cors_resp({'error': f'JSON格式错误: {e}'}, 400)
    ...
    def stream_gen():
        ...
        # 修复：统一读取 'params' 键（agent.py返回的是params而非parsed_params）
        try:
            pref, _ = UserPreference.objects.get_or_create(session_id=session_id)
            parsed = {}
            if route_data:
                # 优先读 'params'，兼容旧版 'parsed_params'
